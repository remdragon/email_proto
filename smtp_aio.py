...
class Client:
	...
	async def connect ( self, hostname: str, port: int ) -> None:
		rx, tx = await asyncio.open_connection ( hostname, port )
		await self._connect ( rx, tx )
	...
	async def _recv ( self, request: smtp.Request ) -> smtp.Response:
		...
		while not request.response:
			data: bytes = await self.rx.readline()
	...
	async def run ( self,
		...
		try:
			...
			async def _send ( data: bytes ) -> None:
				log.debug ( f'S>{b2s(data).rstrip()}' )
				tx.write ( data ); await tx.drain()
			
			async def _recv() -> bytes:
				#try:
				data = await rx.readline()
